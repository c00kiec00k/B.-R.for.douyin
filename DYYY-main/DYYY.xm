...
- (NSArray *)dataArray {
    ...
    NSString *apiKey = [[NSUserDefaults standardUserDefaults] objectForKey:@"DYYYInterfaceDownload"];
    if (apiKey.length > 0) {
        AWELongPressPanelBaseViewModel *apiDownloadAction = [[%c(AWELongPressPanelBaseViewModel) alloc] init];
        apiDownloadAction.awemeModel = self.awemeModel;
        apiDownloadAction.actionType = 673;
        apiDownloadAction.duxIconName = @"ic_cloudarrowdown_outlined_20";
        apiDownloadAction.describeString = @"接口保存视频";
            
        apiDownloadAction.action = ^{
            NSString *shareLink = [self.awemeModel valueForKey:@"shareURL"];
            if (shareLink.length == 0) {
                [DYYYManager showToast:@"无法获取分享链接"];
                return;
        ...
        [viewModels addObject:apiDownloadAction];
    }
...
// 处理Base64备份和恢复功能
// 移除UISwitch的钩子，它会与其他钩子冲突
// 我们改为在DYYYSettingViewController.m中处理
